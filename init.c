...
app_init_rings(void) {
	...
	snprintf(addr_name, sizeof(addr_name), "app_addr_ring_rx_%u", i);

        app.addr_rings_rx[i] = rte_ring_create(
            addr_name,
            app.ring_rx_size,
            rte_socket_id(),
            RING_F_SP_ENQ | RING_F_SC_DEQ);

        if (app.addr_rings_rx[i] == NULL)
            rte_panic("Cannot create RX addr_ring %u\n", i);
